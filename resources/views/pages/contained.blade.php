        ...
        <script>
            ...
            document.onreadystatechange = function () {
              if (document.readyState == "complete") {
                ...
                window.reader = ePubReader("/storage/{{ $file->href }}");
				var hash = "{{ \App\Read::where('user_id', Auth::user()->id)->where('book_id', $book->id)->first()->page ?? '' }}";
				if (hash)
				window.location.hash = hash;
				var test=window.location.hash.slice(1);
				if(test)
				window.reader.book.goto(test);
				function hashHandler(){
					...
					var detect = function(){
						if(that.oldHash!=window.location.hash){
							axios.post("/change_page/{{ $book->id }}", {
